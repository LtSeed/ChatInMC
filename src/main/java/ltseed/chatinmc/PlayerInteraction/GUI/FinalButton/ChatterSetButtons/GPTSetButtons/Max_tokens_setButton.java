...
public class Max_tokens_setButton extends Button {
    private final GUIWithChatterInformation gui;

    public Max_tokens_setButton(GUIWithChatterInformation gui, List<String> lore) {
        super((short) 2, (short) 6,
                Material.PAPER,
                "max_tokens：" + gui.getMax_tokens(), lore);
        this.gui = gui;
    }
    ...
    public void call(Player player) {
        new PlayerConversation(player).startConversation("请输入max_tokens！", response -> {
            try {
                gui.setMax_tokens(Integer.parseInt(response));
